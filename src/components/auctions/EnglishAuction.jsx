...
function EnglishAuction({ newAuctionObj }) {
  ...
  useEffect(() => {
    socket.on('displayBidWinner', (calculatedWinner) => {
      setBidWinner(calculatedWinner);
    });
  }, [auctionTimer]);
  ...
  const setBidAmt = () => {
    ...
    } else if (currentBid > parseInt(bidAmount, 10)) {
      const bidInfo = {
            ...
            { previousBidDetails
                  ...
                  <div>
                    <p>
                      Last Bid:
                      {' '}
                      {previousBidDetails.bidAmount}
                    </p>
                    <p>
                      Bid by:
                      {' '}
                      {previousBidDetails.bidTeam}
                    </p>
                  </div>
                  )}
            <div>
